...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => [str.substring(1, 4), str.substring(6, 9), str.substring(10)].join(''));
};
...
const allHappy = (arr) => {
  let output = true;
  arr.forEach(val => val.includes(':)') ? 'true' : output = false);
  return output;
};
...
const findEvery = (arr, target) => {
  let output = true;
  arr.forEach(val => val.includes(target) ? 'true' : output = false);
  return output;
};
...
const unenrollBrook = (arr) => {
  return arr.map(cls => cls.filter(str => !str.includes('Brook')));
};
